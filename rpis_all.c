...
void read_dht11() {
    ...
    MYSQL_ROW row_hi, row_lo;

    // select max
    if (mysql_query(conn, "SELECT max FROM user_input WHERE type='temperature'")) {}

    MYSQL_RES *max_temp = mysql_store_result(conn);
    ...
    row_hi = mysql_fetch_row(max_temp);

    if(row_hi[0]) {
        hi_temp = atof(row_hi[0]);
    }

    // select min
    if (mysql_query(conn, "SELECT min FROM user_input WHERE type='temperature'")) {}

    MYSQL_RES *min_temp = mysql_store_result(conn);
    ...
    row_lo = mysql_fetch_row(min_temp);

    if(row_lo[0]) {
        lo_temp = atof(row_lo[0]);
    }
	...
	*/
	...
	if((bit_cnt >= 40) &&
		...
		rh = dht11_dat_0[0] + dht11_dat_0[1] / 10.0;
		temp = dht11_dat_0[2] + dht11_dat_0[3] / 10.0;
		char SQLString[64];
    ...
    // deal with temp
    if(temp > 0) {
        if((temp < lo_temp || temp > hi_temp) && relay_on == false) {
            pin_on();
	    mysql_query(conn, "UPDATE device_status SET status='on' WHERE device='AC'");
	    
        }
        else if((temp >= lo_temp && temp <= hi_temp) && relay_on == true) {
            pin_off();
	    mysql_query(conn, "UPDATE device_status SET status='off' WHERE device='AC'");
        }
    }
